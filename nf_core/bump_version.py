...
def bump_pipeline_version(lint_obj, new_version):
    ...
    nfconfig_pattern = r"docker pull nfcore/{name}:dev && docker tag nfcore/{name}:dev nfcore/{name}:(?:{tag}|dev)".format(name=lint_obj.pipeline_name.lower(), tag=current_version.replace('.',r'\.'))
    nfconfig_newstr = "docker pull nfcore/{name}:dev && docker tag nfcore/{name}:dev nfcore/{name}:{tag}".format(name=lint_obj.pipeline_name.lower(), tag=docker_tag)
