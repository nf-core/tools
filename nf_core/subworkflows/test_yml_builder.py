...
class SubworkflowTestYmlBuilder(ComponentCommand):
    ...
    def build_single_test(self, entry_point):
        ...
        while ep_test["command"] == "":
            default_val = f"nextflow run ./tests/subworkflows/{self.modules_repo.repo_path}/{self.subworkflow} -entry {entry_point} -c ./tests/config/nextflow.config -c ./tests/subworkflows/{self.modules_repo.repo_path}/{self.subworkflow}/nextflow.config"
            if self.no_prompts:
