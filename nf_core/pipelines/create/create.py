...
class PipelineCreate:
    ...
    def render_template(self) -> None:
        ...
        # Render the metro-map images from assets/metro_map.mmd, if present
        self.make_metro_map()
    ...
    def make_metro_map(self):
        ...
        try:
            subprocess.run(
                ["nf-metro", "render", str(mmd), "--theme", "nfcore", "--embed-font"],
                check=True,
                capture_output=True,
                text=True,
            )
            ...
            log.warning(
                f"Could not render metro-map images from '{mmd}': {stderr or e}\n"
                "They will be generated by the metro-map GitHub Action on the first push."
            )
