...
def plugin_includes(self) -> Dict[str, List[str]]:
    ...
    workflow_files = [file for file in glob.glob(f"{self.wf_path}/**/*.nf", recursive=True) if not file.startswith("./modules/")]
    test_passed = True
    for file in workflow_files:
        with open(file, "r") as of:
            plugin_includes = re.findall(plugin_include_pattern, of.read())
            for include in plugin_includes:
                if include not in ["nf-validation", "nf-schema"]: continue
                if include != validation_plugin:
                    test_passed = False
                    failed.append(f"Found a `{include}` plugin import in `{file[2:]}`, but `{validation_plugin}` was used in `nextflow.config`")

    if test_passed: passed.append("No wrong validation plugin imports have been found")
