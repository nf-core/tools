...
AUTOGENERATED_MARKER = "// AUTOGENERATED CONFIG BELOW THIS POINT - DO NOT EDIT"
...
def _split_manual_prefix(text: str) -> tuple[str, str]:
    """Split *text* at ``AUTOGENERATED_MARKER``.

    Returns ``(manual_prefix, generated_section)``. *manual_prefix* is
    everything before the marker line, normalised to end with a newline (or
    be empty). *generated_section* is everything after the marker line.

    When the marker isn't present, the whole text is treated as generated
    content and *manual_prefix* is empty. This matches the tool's behaviour
    from before this convention existed, so files written by older versions
    are fully regenerated the first time rather than misread as hand-written.
    """
    marker_index = text.find(AUTOGENERATED_MARKER)
    if marker_index == -1:
        return "", text
    prefix = text[:marker_index]
    if prefix and not prefix.endswith("\n"):
        prefix += "\n"
    rest = text[marker_index + len(AUTOGENERATED_MARKER) :]
    if rest.startswith("\n"):
        rest = rest[1:]
    return prefix, rest


def _read_manual_prefix(config_path: Path) -> str:
    """Return the hand-written content above ``AUTOGENERATED_MARKER``, if any."""
    with contextlib.suppress(OSError):
        return _split_manual_prefix(config_path.read_text())[0]
    return ""


def _parse_config_file(config_path: Path) -> dict[str, str]:
    """Return {module_name: container} from the generated section of an existing platform config file.

    Hand-written entries above ``AUTOGENERATED_MARKER`` are deliberately excluded: they
    are not something a single-module update should treat as a generated entry to
    splice over.
    """
    result: dict[str, str] = {}
    with contextlib.suppress(OSError):
        _, generated_section = _split_manual_prefix(config_path.read_text())
...
def _write_platform_config(config_path: Path, entries: dict[str, str], key: str) -> bool:
    """Write *entries* below ``AUTOGENERATED_MARKER`` in *config_path*.

    Hand-written content above the marker (e.g. overrides for modules without
    per-platform container metadata yet) is preserved as-is. The file is deleted
    only when there are no entries and no hand-written content left. Skips the
    write when content is unchanged. Returns True if the file now exists.
    """
    manual_prefix = _read_manual_prefix(config_path)

    if not entries and not manual_prefix:
        config_path.unlink(missing_ok=True)
        return False

    generated_section = "".join(
        f"process {{ withName: '{name}' {{ {key} = '{container}' }} }}\n" for name, container in sorted(entries.items())
    )
    new_content = f"{manual_prefix}{AUTOGENERATED_MARKER}\n{generated_section}"
...
class ContainerConfigs:
    ...
    def generate_container_configs(self) -> set[str]:
        ...
        modules may have changed. Any hand-written content above
        ``AUTOGENERATED_MARKER`` in each platform config is preserved; only the
        generated section below the marker is replaced, so a removed module's
        entry disappears from there without touching manual overrides.
