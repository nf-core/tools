...
class LabelledProcess(Screen):
    """Get labelled process resource requirements."""
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Markdown(
            dedent(
                """
                # Custom process resources by label
                """
            )
        )
        yield TextInput(
            ...
            try:
                ...
                if event.button.id == "another":
                    ...
                    for tmp_config in self.config_stack:
                        process_label = tmp_config.get('custom_process_name')
                        new_config[key][process_label] = tmp_config
