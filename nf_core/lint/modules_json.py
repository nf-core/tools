...
def modules_json(self):
    ...
    * Failure: If module entries are found in ``modules.json`` for modules that are not installed
    ...
    modules_dir = Path(self.wf_path, "modules", "nf-core")
    ...
    if _modules_json:
        ...
        for repo in modules_json_dict["repos"].keys():
            ...
            if "modules/nf-core" not in modules_json_dict["repos"][repo] or not repo.startswith("http"):
                failed.append(
            ...
            for module, module_entry in modules_json_dict["repos"][repo]["modules/nf-core"].items():
                if not Path(modules_dir, module).exists():
                    failed.append(
                        f"Entry for `{Path(modules_dir, module)}` found in `modules.json` but module is not installed in "
                        "pipeline."
                    )
                    all_modules_passed = False
                if module_entry.get("branch") is None:
                    failed.append(f"Entry for `{Path(modules_dir, module)}` is missing branch information.")
                if module_entry.get("git_sha") is None:
                    failed.append(f"Entry for `{Path(modules_dir, module)}` is missing version information.")
