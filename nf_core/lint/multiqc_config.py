...
def multiqc_config(self):
    ...
    if "report_comment" not in ignore_configs:
        ...
        report_comments = [
            ...
            f'This report has been generated by the <a href="https://github.com/nf-core/{self.pipeline_name}/tree/dev" '
            f"analysis pipeline. For information about how to "
            f'interpret these results, please see the <a href="https://nf-co.re/{self.pipeline_name}/dev/docs/output" '
            'target="_blank">documentation</a>.',
        ]
        try:
            if "report_comment" not in mqc_yml:
                raise KeyError()
            if (version != "dev" and mqc_yml["report_comment"].strip() != report_comments[0]) or (
                version == "dev" and mqc_yml["report_comment"].strip() != report_comments[1]
            ):
                raise AssertionError()
        except (AssertionError, KeyError, TypeError) as e:
            if isinstance(e, AssertionError):
                # find where the report_comment is wrong and give it as a hint
                hint = report_comments[0] if version != "dev" else report_comments[1]
                failed.append(
