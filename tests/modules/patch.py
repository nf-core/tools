...
def test_create_patch_update_success(self):
    ...
    # Update the module
    update_obj = nf_core.modules.ModuleUpdate(
        self.pipeline_dir, sha=SUCCEED_SHA, show_diff=False, recursive=True, remote_url=GITLAB_URL, branch=PATCH_BRANCH
    )
...
def test_create_patch_update_fail(self):
    ...
    update_obj = nf_core.modules.ModuleUpdate(
        self.pipeline_dir, sha=FAIL_SHA, show_diff=False, recursive=True, remote_url=GITLAB_URL, branch=PATCH_BRANCH
    )
