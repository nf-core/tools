...
def test_mod_json_update(self):
    ...
    mod_json_obj.update(mod_repo_obj, "MODULE_NAME", "GIT_SHA", "modules", False)
    mod_json = mod_json_obj.get_modules_json()
    ...
    assert "modules" == mod_json["repos"][NF_CORE_MODULES_REMOTE]["modules"]["nf-core"]["MODULE_NAME"]["installed"]
...
def test_mod_json_up_to_date_reinstall_fails(self):
    ...
    mod_json_obj.update(ModulesRepo(), "fastqc", "INVALID_GIT_SHA", "modules", True)
